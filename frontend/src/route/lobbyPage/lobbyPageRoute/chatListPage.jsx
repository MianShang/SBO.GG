...
function ChatListPage({ selectedRoom, setSelectedRoom, setMessages }) {
  ...
  const getChatList     = useChatListGet(setMessages);          // 실시간으로 구독한 채팅방의 채팅 목록 가져오는 커스텀훅
  ...
  return (
    <div className='listRouteSize contentStyle' style={{ color:"white" }}>
      ...
      { chatList.map((item, i) => (
        ...
        <div key={item.id} className={'chatListStyle'} onClick={()=>{ setSelectedRoom(item.chatRoom);  getChatList(item.chatRoom.id); }}>
            
          {/* 스타일 임시로 지정 */}
          <div style={{display:"flex", textAlign:"center"}}>

            
            <p onClick={()=>{ }}> {item.chatRoom.name} </p>
